...
public class TSIClientAuthenticator implements ClientAuthenticator, ClientAuthenticatorFactory {
    ...
    private static final Logger logger = Logger.getLogger(TSIClientAuthenticator.class);
...
/*
    ...
    public void authenticateClient(ClientAuthenticationFlowContext context) {
        logger.warnf("In TSI authenticateClient");
        System.out.println("TSI AuthenticateClient");
        context.success();
        return;
    ...
    */
    ...
    public void authenticateClient(ClientAuthenticationFlowContext context) {
        logger.warnf("In TSI authenticateClient");
        System.out.println("TSI AuthenticateClient with value test");
        //context.getClientAuthAttributes().put("tsitest","tsivalue");
        //context.success();

        MultivaluedMap<String, String> params = context.getHttpRequest().getDecodedFormParameters();

        String tsiToken = params.getFirst("tsi_token");

        if (tsiToken == null) {
            Response challengeResponse = ClientAuthUtil.errorResponse(Response.Status.BAD_REQUEST.getStatusCode(), "invalid_client", "Parameter tsi_token is missing");
            context.challenge(challengeResponse);
            return;
        }
        ...
        */
        ...
        try {
            JWSInput jws = new JWSInput(tsiToken);
            JsonWebToken token = jws.readJsonContent(JsonWebToken.class);
/*
            RealmModel realm = context.getRealm();
            String clientId = token.getSubject();
    ...
    @Override
    public TSIClientAuthenticator create() {
        logger.warnf("TSIClientAuthenticator create()");
        System.out.println("TSIClientAuthenticator create()");
    ...
    @Override
    public TSIClientAuthenticator create(KeycloakSession session) {
        logger.warnf("TSIClientAuthenticator create(session)");
        System.out.println("TSIClientAuthenticator create(session)");
    ...
    @Override
    public void postInit(KeycloakSessionFactory factory) {
        logger.warnf("TSIClientAuthenticator postInit");
        System.out.println("TSIClientAuthenticator postInit");
    ...
    @Override
    public void init(Config.Scope config) {
        logger.warnf("TSIClientAuthenticator init(config)");
        System.out.println("TSIClientAuthenticator init(config)");
    }
}
